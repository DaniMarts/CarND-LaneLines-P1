...
def draw_lines(img, vertices, lines, color=[255, 0, 0], thickness=2):
    ...
    # It will find the weihed average of the slopes of the lane lines
    ...
    right_lines_data = lines_data[:, slopes > 0]
    left_lines_data = lines_data[:, slopes < 0]

    # just for more readability
    r_slopes, r_lengths, r_y_offsets = right_lines_data
    l_slopes, l_lengths, l_y_offsets = left_lines_data
    ...
    right_lines_data = right_lines_data[:, r_m + 6*(r_uq - r_m) < r_slopes < r_m - 6*(r_m - r_lq)]
    left_lines_data = left_lines_data[:, l_m + 6*(l_uq - l_m) < l_slopes < l_m - 6*(l_m - l_lq)]
    ...
    r_x1 = (y1 - r_y_offset)/r_slope
    r_x2 = (y2 - r_y_offset)/r_slope

    l_x1 = (y1 - l_y_offset)/l_slope
    l_x2 = (y2 - l_y_offset)/l_slope        

    # drawing the lines from top to bottom on a black image
    black_image = np.zeros((*img.shape, 3))  # a black image with 3 color channels
...
for blured_image in blurred:
    cannyed_image = canny(blured_image, 100, 200)
...
vertices = np.array([[(round(0.08*x_size), y_size), (round(0.45*x_size), round(0.60*y_size)), (round(0.55*x_size), round(0.60*y_size)), (round(0.98*x_size), y_size)]])
...
for image in focused:
    lines = cv2.HoughLinesP(image, rho=1, theta=1/180*np.pi, threshold=10, minLineLength=100, maxLineGap=100)
    # marked_lanes.append(line_image)
    print(lines.shape)
    line_image = draw_lines(image, vertices, lines)
    plt.figure()
...
for image in marked_lanes:
    final_lane = draw_lines(image, vertices, lines)
...
def process_image(image):
    ...
    lanes, lines = hough_lines(region_of_interest(canny(gaussian_blur(grayscale(image), 5), 100, 200), vertices), rho=1, theta=1/180*np.pi, threshold=10, min_line_len=100, max_line_gap=100)
